	        ...
	        <ul id="navlist" class="nav navbar-primary navbar-nav pull-md-right">
				...
				<li class="nav-item">
	                ...
	                <ul id="parciales" class="dropdown-menu">
	                    <li><a href="cdeparcial1.php">Parcial 1</a></li>
 ...
 <div class="container c-principal">
  <div class="row p-contenido">
	<div class="col-xl-2 col-lg-4 col-md-4 col-xs-12 sidebar">
		...
		<form class="w3-container w3-card-4 w3-padding-16 w3-white" action="cdeparcial1.php" method="post">
	      <div class="w3-section"> 
	    	<label>Accion a realizar:</label>
		  	<select id="tipou" name="tipou" type="hidden">
			  <option value="0">Asistencia Grupal</option>
			  <option value="1">Asistencia Individual</option>
			  <option value="2">Mostrar Asistencias</option>
		    </select>
		  </div>
    	
          <button type="submit" name="submit1" >Siguiente</button>
       </form>
	...
	<div class="col-xl-12 col-lg-12 col-md-12 col-xs-12 main" align="center"> -->
		...
		if(isset($_POST['submit1'])){
			...
			$sql2 = "SELECT * FROM $tabla where numentrenador='$numero' and status='0'";
 	   ...
 	   <?php
  		...
  		?>
			   ...
			   <th><?php echo $row2["asistenciasp1"];?></th>
    			<th> <input  type="checkbox" name="asistencias[]" value="<?php echo  $row2["numero"] ?> " > </th>	
  			  </tr>
    	...
    	<?php
		 ...
		 ?>
				...
				<input type="hidden" name="tabla" value="<?php echo $tabla; ?>" >
			 	<button type="submit" name="submit2" class="button">Tomar Asistencia</button>
       		</form>
		 <?php
	...
	if(isset($_POST['submit2'])){
		...
		if(!empty($_POST['asistencias'])) {
			...
			$checked_contador = count($_POST['asistencias']);
			echo "<p>Asistieron a la clase los siguientes ".$checked_contador." deportistas:</p> <br/>";
			// Bucle para almacenar y visualizar valores activados checkbox.
			foreach($_POST['asistencias'] as $seleccion) {
				//echo "<p>".$seleccion ."</p>";
				
				
				$sql2 = mysqli_query ($link, "SELECT * FROM $tabla where numero='$seleccion' and status=0");
				//$result = $link->query($sql);
				if ($sql2->num_rows > 0) {
  					// output data of each row
  					while($row = $sql2->fetch_assoc()) {
						$nasistencias=$row["asistenciasp1"];
					}
				} else {
  					echo "No existe el usuario en la base de datos. Vuelva a escoger otro usuario";
	 			}	
				
				$nasistencias = $nasistencias+1;
				
				// 2.- CONEXION A LA BASE DE DATOS
				$link = new mysqli($db_host, $db_login, $db_pswd, $db_name);
			    $query="update deportistas set asistenciasp1='$nasistencias' WHERE numero='$seleccion'";
